const fs = require('fs');
// const pathToFfmpeg = require('ffmpeg-ffprobe-static').ffmpegPath;
// const pathToFfprobe = require('ffmpeg-ffprobe-static').ffprobePath;
...
// 转换评论为 ASS 格式
function convertCommentsToAss(comments, outputFile, width = 1920, height = 1080, fontface = 'Arial', fontsize = 50, alpha = 0.8, duration = 10) {
...
// 生成弹幕 ASS 文件
async function generateDanmuAss(commentID, filePath, width = 1920, height = 1080, fontface = 'Arial', fontsize = 50, alpha = 0.8, duration = 6) {
...
async function combineSubAss(sub1, sub2) {
    ...
    if (path.extname(sub2).toLowerCase() === '.ass') {
        ...
        const updatedStyleLines = styleLines.map(line => {
        ...
        fs.writeFileSync(output, `${sub1Content}\n[V4+ Styles]\n${formatLine}\n${updatedStyleLines}\n[Events]\n${eventsContent}`);
...
export const manualMatch = async (episodeID, filePath) => {
  ...
  try {
      const response = await axios.post(url, {
          ...
          headers: {
              Accept: 'application/json',
              Authorization: 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJBcHBJZCI6ImRkcGxheXdpbjMydjMiLCJVc2VySWQiOiIzMjk1NjkiLCJUb2tlbiI6IjEwNzc5ODA1NzAiLCJUaW1lc3RhbXAiOiIxNzIxNTY5MTgxIiwiZXhwIjoxNzIzMzgzNTgxLCJpc3MiOiJkYW5kYW5wbGF5Lm5ldCIsImF1ZCI6ImRhbmRhbnBsYXkubmV0In0.33pkIVc0MiHZ-_2Ml65DSSFvrcd1MXjxkM0_p6ya584'
          }
      });
